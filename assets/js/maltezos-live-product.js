(() => {
  ...
  function gallery(images,name){
    ...
    if(!main||!grid)return;
    grid.innerHTML='';
    if(!list.length){main.style.display='none';if(fallback)fallback.style.display='';return}
    const setMain=(src,btn)=>{main.src=src;main.alt=name;main.style.display='block';if(fallback)fallback.style.display='none';grid.querySelectorAll('.thumb-btn').forEach(b=>b.classList.remove('active'));btn?.classList.add('active')};
    list.slice(0,8).forEach((src,i)=>{const b=document.createElement('button');b.type='button';b.className='thumb-btn'+(i===0?' active':'');const im=document.createElement('img');im.src=src;im.alt=`${name} ${i+1}`;b.appendChild(im);b.addEventListener('click',()=>setMain(src,b));grid.appendChild(b)});
  ...
  function render(){
    ...
    if(name)name.textContent=c.name;if(desc)desc.textContent=c.description;
    ...
    if(badge){badge.classList.remove('stock-low','stock-medium','stock-high');badge.classList.add(stockClass(n));const label=badge.querySelector('.stock-label'),value=badge.querySelector('.stock-value');if(label)label.textContent=c.stock;if(value)value.textContent=`${n} ${c.unit(n)}`}
    gallery(product.images,c.name);pricing(product,c);
  }
  ...
  async function load(){
    ...
    try{
      ...
      if(error)throw error;if(!data){unavailable();return}product=data;render();
